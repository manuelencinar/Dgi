  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px 80px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, gap: 12, flexWrap: 'wrap' }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: 'var(--text-strong)' }}>Notificaciones</h1>
        {unread > 0 && <button onClick={markAll} style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.25)', padding: '7px 14px', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit' }}>Marcar todas como leídas</button>}
      </div>

      {items.length === 0 ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '60px 20px', textAlign: 'center' }}>
          <p style={{ fontSize: 32, marginBottom: 12 }}>🔔</p>
          <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>No tienes notificaciones todavía.</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gap: 8 }}>
          {items.map(n => (
            <Link key={n.id} href={n.ticker ? `/empresa/${encodeURIComponent(n.ticker)}` : '#'} style={{
              ...
              background: n.read ? 'var(--surface)' : 'rgba(99,102,241,0.07)',
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <p style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 4 }}>{timeAgo(n.created_at)}</p>
              </div>
