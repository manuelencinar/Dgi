              ...
              <thead>
                <tr>
                  {['Fecha estimada', 'Importe estim.', 'Tipo'].map((h, i) => (
                    <th key={h} style={{ padding: '6px 8px', textAlign: i === 0 ? 'left' : 'right', color: '#4a5270', fontWeight: 600, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                ...
                {dcf.fcfYears.map(y => (
                  <div key={y.year} style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ fontSize: 12, fontWeight: 700, color: y.value >= 0 ? '#34d399' : '#f87171' }}>
                      {Math.abs(y.value) >= 1e9 ? (y.value/1e9).toFixed(1)+'B' : Math.abs(y.value) >= 1e6 ? (y.value/1e6).toFixed(0)+'M' : y.value.toFixed(0)}
                    </p>
                  </div>
                ))}
              </div>
              ...
              return (
                <div key={y.year} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 600, color: col, width: 70, textAlign: 'right', flexShrink: 0 }}>
                    {y.isBuyback ? '−' : '+'}{absM >= 1000 ? `${(absM / 1000).toFixed(1)}B` : `${absM.toFixed(0)}M`}
                  </span>
