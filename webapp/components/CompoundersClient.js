...
function roicColor(r) { if (r >= 30) return '#34d399'; if (r >= 22) return '#86efac'; return '#fbbf24' }

const ROW_CSS = `
.cmp-row{display:flex;align-items:center;gap:8px;padding:5px 11px;background:rgba(255,255,255,0.02);border-radius:8px;margin-bottom:2px}
...
.cmp-name{font-size:13px;font-weight:700;color:#d0d8e8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.3}
.cmp-ticker{font-size:10px;color:#2e3a55;font-weight:600}
.cmp-m{display:none;flex-direction:column;text-align:right;min-width:62px;flex-shrink:0}
.cmp-mlabel{font-size:9px;color:#3a4260;font-weight:400}
.cmp-mval{font-size:13px;font-weight:700;color:#8090a8;font-variant-numeric:tabular-nums}
...
@media(min-width:760px){
  ...
  .cmp-rank{display:inline;width:24px;text-align:right;font-size:12px;font-weight:800;color:#3a4260;flex-shrink:0}
    ...
    <Link href={`/empresa/${encodeURIComponent(co.t)}`} style={{ textDecoration: 'none', display: 'block' }}>
      <div className="cmp-row">
        ...
        <div className="cmp-m" title="CapEx sobre flujo de caja operativo — cuanto más bajo, menos capital necesita el negocio">
          ...
          <span className="cmp-mval" style={{ color: co.capex <= 10 ? '#34d399' : '#86efac' }}>{co.capex.toFixed(0)}%</span>
        </div>
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px 100px' }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: '#e0e8f0', marginBottom: 4 }}>⚙️ Máquinas de Compounding</h1>
        <p style={{ fontSize: 12, color: '#5a6480', lineHeight: 1.5 }}>
          El santo grial de la calidad: negocios con <strong style={{ color: '#8090a8' }}>ROIC alto y sostenido</strong> que <strong style={{ color: '#8090a8' }}>apenas necesitan reinvertir capital</strong> (CapEx/CFO &lt; 20%) y crecen de forma estable — generan caja y componen valor año tras año.
        </p>
      </div>
      ...
      <p style={{ fontSize: 12, color: '#4a5270', marginBottom: 10 }}>
        <span style={{ fontWeight: 700, color: '#c8d0e0', fontSize: 14 }}>{filtered.length.toLocaleString('es-ES')}</span> máquinas de compounding (ROIC ≥18% · CapEx/CFO ≤20% · crecimiento ≥4%)
      </p>
        ...
        <>
          ...
          {!isPremium && hidden > 0 && (
            <div style={{ marginTop: 12, padding: '18px 20px', textAlign: 'center', background: 'rgba(99,102,241,0.06)', border: '1px dashed rgba(99,102,241,0.3)', borderRadius: 12 }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: '#c8d0e0', marginBottom: 4 }}>+ {hidden} máquinas de compounding más</p>
              <p style={{ fontSize: 12, color: '#4a5270', marginBottom: 14 }}>Premium desbloquea el ranking completo.</p>
