...
const TYPE_META = {
  ...
  etf:     { icon: '📊', label: 'ETF',     color: 'var(--positive)' },
  fund:    { icon: '📦', label: 'Fondo',   color: 'var(--positive)' },
  index:   { icon: '🌐', label: 'Índice',  color: 'var(--warning)' },
}
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
              ...
              <button onClick={close} aria-label="Cerrar" style={{ background: 'none', border: 'none', color: 'var(--text-faint)', fontSize: 14, cursor: 'pointer', padding: 2 }}>✕</button>
            </div>
            ...
            <div style={{ maxHeight: '52vh', overflowY: 'auto' }}>
              ...
              {q.trim().length < 1 && (
                <div style={{ padding: '22px 16px', textAlign: 'center', color: 'var(--text-faintest)', fontSize: 12.5 }}>
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ flex: 1, minWidth: 0 }}>
                      <span style={{ display: 'block', color: 'var(--text-strong)', fontSize: 13.5, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</span>
                      {item.sub && <span style={{ display: 'block', color: 'var(--text-faint)', fontSize: 11.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.sub}</span>}
                    </span>
                    <span style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', fontVariantNumeric: 'tabular-nums' }}>{item.ticker}</span>
            ...
            {/* Pie */}
            <div style={{ padding: '8px 14px', borderTop: '1px solid var(--border)', display: 'flex', gap: 14, fontSize: 10.5, color: 'var(--text-faintest)' }}>
              <span>↑↓ navegar</span>
              <span>↵ abrir</span>
