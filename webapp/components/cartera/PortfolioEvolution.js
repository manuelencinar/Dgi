...
const BLUE = '#60a5fa', GREEN = 'var(--positive)', ORANGE = 'var(--warning)', RED = 'var(--negative)', TEAL = '#22d3ee'
  ...
  return (
    <div style={{ ...CARD, marginBottom: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 14 }}>
        <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Evolución del patrimonio</p>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <select value={year} onChange={e => setYear(Number(e.target.value))} disabled={!isPremium}
            style={{ background: 'var(--surface-2)', border: '1px solid var(--border-strong)', borderRadius: 7, padding: '6px 10px', color: isPremium ? 'var(--text)' : 'var(--text-faint)', fontSize: 12, outline: 'none', cursor: isPremium ? 'pointer' : 'not-allowed' }}>
      ...
      {k && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10, marginBottom: 16 }}>
          <Kpi label="Valor actual" value={fmtEUR(kCurrentValue)} sub={k.contributedYear ? `Aportado en ${year}: ${k.contributedYear >= 0 ? '+' : ''}${fmtEUR(k.contributedYear)}` : null} subCol="var(--text-muted)" col="var(--text-strong)" />
          <Kpi label="Capital invertido" value={fmtEUR(kInvested)} sub="Coste medio ponderado" col="var(--text)" />
      ...
      {loading ? (
        <div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-faint)', fontSize: 13 }}>Calculando evolución…</div>
      ) : data?.flags?.empty || !chartData.length ? (
        <div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-faint)', fontSize: 14 }}>Sin operaciones en {year}</div>
      ) : (
        <>
          <ResponsiveContainer width="100%" height={height}>
            <ComposedChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis stroke="var(--text-faint)" fontSize={10} tickLine={false} tickFormatter={mode === 'pct' ? (v => `${v}%`) : axisEUR} />
              <Tooltip content={<EvoTooltip year={year} mode={mode} hasDivs={hasDivs} />} cursor={{ fill: 'var(--surface-2)' }} />

              {mode === 'eur' ? (
                <>
                  <Bar dataKey="mv" radius={[3, 3, 0, 0]} maxBarSize={48}>
                    {chartData.map((d, i) => <Cell key={i} fill={d.noData ? 'var(--surface-3)' : d.positive ? BLUE : RED} fillOpacity={0.85} />)}
                  </Bar>
                ...
                <>
                  ...
                  <Bar dataKey="retPct" radius={[3, 3, 0, 0]} maxBarSize={48}>
                    {chartData.map((d, i) => <Cell key={i} fill={d.noData ? 'var(--surface-3)' : d.retPct >= 0 ? GREEN : RED} fillOpacity={0.85} />)}
                  </Bar>
          ...
          {/* Leyenda */}
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginTop: 8, fontSize: 10.5, color: 'var(--text-faint)', alignItems: 'center' }}>
            ...
            {hasDivs && (
              <button onClick={() => setShowDivs(s => !s)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: showDivs ? 'var(--warning)' : 'var(--text-faintest)', fontSize: 10.5, padding: 0, display: 'inline-flex', alignItems: 'center' }}>
            ...
            {hasRealized && (
              <span style={{ color: 'var(--text-faint)' }}><span style={{ display: 'inline-block', width: 12, height: 0, borderTop: `2px dotted ${TEAL}`, marginRight: 5, verticalAlign: 'middle' }} />Resultado realizado</span>
            )}
          </div>

          {data?.flags?.usedFallback && (
            <p style={{ fontSize: 9.5, color: 'var(--text-faintest)', marginTop: 8 }}>
...
const btn = active => ({
  ...
  color: active ? 'var(--accent)' : 'var(--text-faint)', fontWeight: active ? 700 : 400,
})

function Kpi({ label, value, sub, subCol, col, badge }) {
  return (
    <div style={{ background: 'var(--surface)', borderRadius: 8, padding: '12px 14px', border: '1px solid var(--surface-3)' }}>
      <p style={{ fontSize: 10, color: 'var(--text-faint)', marginBottom: 4 }}>{label}</p>
      <p style={{ fontSize: 18, fontWeight: 800, color: col }}>{value}</p>
      {badge && <p style={{ fontSize: 9.5, fontWeight: 700, color: 'var(--warning)', marginTop: 3 }}>{badge}</p>}
      {sub && <p style={{ fontSize: 10, color: subCol || 'var(--text-faintest)', marginTop: 3 }}>{sub}</p>}
    </div>
  )
}
...
function EvoTooltip({ active, payload, label, year, mode, hasDivs }) {
  ...
  if (!d) return null
  if (d.noData) return (
    <div style={{ background: 'var(--bg-elev)', border: '1px solid var(--border-strong)', borderRadius: 8, padding: '8px 12px', fontSize: 11 }}>
      <p style={{ color: 'var(--text)', fontWeight: 700 }}>{label} {year}</p>
      <p style={{ color: 'var(--text-faint)' }}>Sin datos para este mes</p>
    </div>
  )
  ...
  return (
    <div style={{ background: 'var(--bg-elev)', border: '1px solid var(--border-strong)', borderRadius: 8, padding: '10px 13px', fontSize: 11.5, minWidth: 200 }}>
      <p style={{ color: 'var(--text)', fontWeight: 700, marginBottom: 6 }}>{label} {year}</p>
      <Row label="Valor de mercado" val={fmtEUR(d.mv)} col="#60a5fa" />
      <Row label="Capital invertido" val={fmtEUR(d.inv)} col="var(--positive)" />
      <Row label="Ganancia latente" val={`${gain >= 0 ? '+' : ''}${fmtEUR(gain)} (${fmtPct(gainPct)})`} col={gain >= 0 ? 'var(--positive)' : 'var(--negative)'} />
      {hasDivs && d.divMonth > 0 && <Row label="Dividendos del mes" val={fmtEUR(d.divMonth, 2)} col="var(--warning)" />}
      {hasDivs && d.divAccum > 0 && <Row label="Dividendos acumulados" val={fmtEUR(d.divAccum, 2)} col="var(--warning)" />}
      {d.realized !== 0 && <Row label="Resultado realizado acum." val={`${d.realized >= 0 ? '+' : ''}${fmtEUR(d.realized, 2)}`} col={d.realized >= 0 ? '#22d3ee' : 'var(--negative)'} />}
      <Row label="Rentab. total (con div.)" val={fmtPct(totalRet)} col="var(--accent)" />
    </div>
  )
}
function Row({ label, val, col }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, marginBottom: 2 }}>
      ...
      <span style={{ color: col || 'var(--text)', fontWeight: 600 }}>{val}</span>
    </div>
  )
}
