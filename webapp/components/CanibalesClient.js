...
.can-name{font-size:13px;font-weight:700;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.3}
.can-ticker{font-size:10px;color:var(--text-faintest);font-weight:600}
.can-m{display:none;flex-direction:column;text-align:right;min-width:54px;flex-shrink:0}
.can-mlabel{font-size:9px;color:var(--text-faintest);font-weight:400}
.can-mval{font-size:13px;font-weight:700;color:var(--text-muted);font-variant-numeric:tabular-nums}
...
@media(min-width:760px){
  ...
  .can-rank{display:inline;width:24px;text-align:right;font-size:12px;font-weight:800;color:var(--text-faintest);flex-shrink:0}
    ...
    <Link href={`/empresa/${encodeURIComponent(co.t)}`} style={{ textDecoration: 'none', display: 'block' }}>
      <div className="can-row">
        ...
        <div className="can-m">
          ...
          <span className="can-mval" style={{ color: 'var(--text-muted)', fontSize: 11 }}>{co.s || '—'}</span>
        </div>
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px 100px' }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: 'var(--text-strong)', marginBottom: 4 }}>🦈 Caníbales de acciones</h1>
        <p style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5 }}>
          Empresas que más han <strong style={{ color: 'var(--text-muted)' }}>reducido sus acciones en circulación desde ~2022</strong> — recompra neta real (en número de acciones), que aumenta tu participación sin que muevas un dedo.
        </p>
      </div>
      ...
      <p style={{ fontSize: 12, color: 'var(--text-faint)', marginBottom: 10 }}>
        <span style={{ fontWeight: 700, color: 'var(--text)', fontSize: 14 }}>{filtered.length.toLocaleString('es-ES')}</span> empresas han reducido acciones desde ~2022
      </p>
        ...
        <>
          ...
          {!isPremium && hidden > 0 && (
            <div style={{ marginTop: 12, padding: '18px 20px', textAlign: 'center', background: 'rgba(99,102,241,0.06)', border: '1px dashed rgba(99,102,241,0.3)', borderRadius: 12 }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>+ {hidden} caníbales más</p>
              <p style={{ fontSize: 12, color: 'var(--text-faint)', marginBottom: 14 }}>Premium desbloquea el ranking completo de caníbales de acciones.</p>
