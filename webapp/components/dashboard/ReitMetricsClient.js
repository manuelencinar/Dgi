...
export default function ReitMetricsClient() {
  ...
  if (reits === null) return <Card><p style={{ fontSize: 13, color: 'var(--text-faint)' }}>Cargando REITs…</p></Card>
  ...
  return (
    <Card>
      ...
      <p style={{ fontSize: 12, color: 'var(--text-faint)', marginBottom: 6 }}>
        El sub-tipo fija el % de capex de mantenimiento sobre NOI que se resta al FFO para estimar el <b style={{ color: 'var(--text-muted)' }}>AFFO</b>. Yahoo no lo distingue bien (los <b style={{ color: 'var(--text-muted)' }}>net-lease</b> salen como "Retail") → corrígelo aquí. El % opcional sobreescribe el del sub-tipo.
      </p>
      {msg && <p style={{ fontSize: 12, color: msg.t === 'ok' ? 'var(--positive)' : 'var(--negative)', marginBottom: 8 }}>{msg.m}</p>}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 620 }}>
          ...
          <tbody>
            {filtered.slice(0, 120).map(b => {
              ...
              return (
                <tr key={b.ticker}>
                  <td style={td}>{b.name} <span style={{ color: 'var(--text-faint)', fontSize: 10 }}>{b.ticker}</span></td>
                  <td style={{ ...td, color: 'var(--text-faint)', fontSize: 11 }}>{b.industry || '—'}</td>
                  <td style={td}>
