...
export default function BankMetricsClient() {
  ...
  if (banks === null) return <Card><p style={{ fontSize: 13, color: 'var(--text-faint)' }}>Cargando bancos…</p></Card>

  return (
    <Card>
      <SectionTitle>Métricas bancarias por trimestre</SectionTitle>
      <p style={{ fontSize: 12, color: 'var(--text-faint)', marginBottom: 14 }}>
        NPL (morosidad) <b style={{ color: 'var(--text-muted)' }}>siempre manual</b> — mientras no lo rellenes, en la ficha sale "–" (nunca 0). NIM/ROTE/eficiencia se calculan solos; aquí solo los rellenas si quieres <b style={{ color: 'var(--text-muted)' }}>sobreescribir</b> el cálculo (bancos sin desglose en Yahoo). Periodo: <code>YYYYQn</code> (p.ej. 2025Q1).
      </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }}>
        ...
        <div className="bm-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }}>
          ...
          <div>
            {!sel ? (
              <p style={{ fontSize: 13, color: 'var(--text-faint)' }}>Selecciona un banco para ver y editar sus trimestres.</p>
            ) : (
              <>
                <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-strong)', marginBottom: 10 }}>{banks.find(b => b.ticker === sel)?.name} <span style={{ color: 'var(--text-faint)', fontSize: 11 }}>{sel}</span></p>
