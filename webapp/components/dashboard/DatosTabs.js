...
const TABS = [
  ['empresas', 'Empresas'], ['sectores', 'Sectores'], ['banca', 'Banca'], ['seguros', 'Seguros'], ['funds', 'ETFs y Fondos'], ['indices', 'Índices'],
  ...
  return (
    <Card>
      ...
      <div style={{ display: 'grid', gap: 1 }}>
        {(logs || []).map((l, i) => (
          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'baseline', padding: '7px 0', borderBottom: '1px solid rgba(255,255,255,0.04)', fontSize: 12 }}>
            <StatusDot status={l.status} />
            <span style={{ flex: 1, color: '#c8d0e0' }}>{l.description || l.event_type}</span>
            <span style={{ color: '#4a5270', fontSize: 10 }}>{l.event_type}</span>
            <span style={{ color: '#4a5270', fontSize: 10, whiteSpace: 'nowrap' }}>{fmtDateTime(l.created_at)}</span>
          </div>
        ))}
        {!logs?.length && <p style={{ fontSize: 12, color: '#4a5270' }}>Sin eventos registrados.</p>}
      </div>
    </Card>
  )
}
...
export default function DatosTabs({ companies, sectors, countries, funds, markets, logs }) {
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      <div style={{ display: 'flex', gap: 4, overflowX: 'auto', borderBottom: '1px solid rgba(255,255,255,0.08)', marginBottom: 16 }}>
        {TABS.map(([id, label]) => {
          const active = id === tab
          return (
            <button key={id} onClick={() => setTab(id)} style={{
              flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', padding: '10px 14px',
              fontSize: 13, fontWeight: 700, color: active ? '#fff' : '#4a5270',
              borderBottom: active ? '2px solid #818cf8' : '2px solid transparent', whiteSpace: 'nowrap',
            }}>{label}</button>
          )
        })}
      </div>
