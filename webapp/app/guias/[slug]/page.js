...
export default async function GuiaPage({ params }) {
  ...
  const g = getGuia(slug)
  if (!g) notFound()
  const related = (g.related || []).map(getGuia).filter(Boolean).slice(0, 3)
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <article style={{ maxWidth: 720, margin: '0 auto', padding: '32px 20px 70px' }}>
        ...
        <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '18px 0 10px' }}>{g.category} · {g.minutes} min de lectura</p>
        <h1 style={{ fontSize: 'clamp(26px, 5vw, 34px)', fontWeight: 900, color: 'var(--text-strong)', lineHeight: 1.2, marginBottom: 22 }}>{g.title}</h1>

        <GuiaContent content={g.content} />

        {/* CTA de registro al pie */}
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {related.map(r => (
                <Link key={r.slug} href={`/guias/${r.slug}`} style={{ ...CARD, textDecoration: 'none', display: 'block' }}>
                  <p style={{ fontSize: 10.5, color: 'var(--accent)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>{r.category}</p>
