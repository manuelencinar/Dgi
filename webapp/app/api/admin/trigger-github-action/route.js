...
const WORKFLOW = 'update_fundamentals.yml'
    ...
    if (res.status === 204) {
      ...
      try {
        await serviceClient().from('admin_logs').insert({
          ...
          description: `Run de yfinance disparado manualmente por ${admin.email}`,
          status: 'ok',
        })
      } catch {}
      return NextResponse.json({ ok: true, message: 'Workflow disparado correctamente' })
    }
