...
- Fuentes (cascada acumulativa): **SEC EDGAR** (`scripts/backfill_sec_edgar.py`, oficial, us-gaap/ifrs-full 10-K/20-F, ~615 empresas US+ADR a ~13-20a — la principal) · **stockanalysis** (`scripts/backfill_stockanalysis.py`, scraping, ~5a no-US) · **Macrotrends** (`scripts/backfill_macrotrends.py`, curl_cffi — **BLOQUEADO por Cloudflare desde datacenter Y desde IP residencial**; requiere navegador headless, valor incremental bajo → descartado de momento) · **ESEF (UE)** y **EDINET (Japón)** oficiales — pendientes de construir (EDINET necesita API key gratuita del usuario).
