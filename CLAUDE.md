...
- Exporta una "hoja de comparación financiera" en PDF: dos columnas con color de marca por posición + chip de sector, tabla comparativa fila a fila, estrellas de dividendo (rentabilidad/seguridad/crecimiento histórico/prob. de seguir subiendo), tabla de puntuación /10 con barras y veredicto "¿cuál compraría hoy?" **derivado solo de datos** (sin frases subjetivas). Disclaimer legal al pie. **Sin logos** (marcas registradas): solo ticker, nombre y color.
- **Generación**: HTML/CSS (string) → **Puppeteer + `@sparticuz/chromium`** en el runtime Node serverless de Vercel. `lib/pdf.js` (`htmlToPdf`) reutiliza una única instancia de navegador entre invocaciones; en local usa el Chrome del sistema (`localExecutable`). `next.config.mjs` → `serverExternalPackages: ['puppeteer-core','@sparticuz/chromium']` (no empaquetar el binario).
- **Datos (fuente única)**: `lib/infografia-data.js` `buildInfographicModels(tickers)` reutiliza `buildComparadorCompanies` (mismos scores/sub-scores/yield) y lo enriquece con `dividendSafety`, `yield_avg` (yield actual vs media histórica) y `shares_reduced_pct` (recompra). **Fiscalidad ES fija** (`ES_DEST_WHT=19`): muestra yield **bruto y neto** (neto = −19% destino + retención en origen del país con tope de doble imposición del 15%, vía `netYield`). Colores de columna por posición (`COLUMN_COLORS`, A índigo / B teal); el sector va como chip en su color de supersector (`sectorInfo`→`SUPERSECTORS`). Veredicto: `buildVerdict` (2 empresas) / `buildSingleVerdict` (1) — solo datos (score, MoS, yield-vs-media, seguridad). NOTA: la puntuación del PDF usa el motor del screener (`computeScore`), coherente con el comparador; puede diferir ligeramente del `computeDGIScore` de la `DGIScoreCard` de la ficha.
