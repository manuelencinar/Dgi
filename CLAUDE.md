...
- `scripts/recalc_valuation.py` — recalc de una vez del `intrinsic_value` en BD desde los estados ya guardados (sin yfinance), reutilizando el mismo `compute_valuation`. Dry-run por defecto; `--write` persiste en lotes; `--ticker X` para una. Ya ejecutado (1889 valoradas / 605 NA; antes 1287 sin valor). Divisa por sufijo de ticker. **Nota**: el screener usa un moat heurístico propio (roic/streak) distinto del `computeMoat` de la ficha → el MoS del screener y el de la ficha pueden diferir ligeramente (pre-existente, no afecta a los outliers, ya resueltos en ambos).
