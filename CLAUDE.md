...
- `scripts/update_prices.py`: descarga masiva de Yahoo a daily_prices + tipos de cambio. Incluye `BENCHMARK_TICKERS = ["^GSPC","^STOXX","URTH","^NDX","^FTSE","^GDAXI","^N225"]`. **`get_all_tickers()` PAGINA** (`_select_all`): sin paginar, PostgREST devolvía solo 1000 de las 2496 empresas y ~1500 tickers NUNCA recibían precio diario (la cartera los mostraba con días de retraso mientras la ficha, que cotiza en vivo, iba al día). Cron: **17:00 UTC** (cierre europeo todo el año; a las 16:00 se capturaba intradía de oct-mar) y 22:00 UTC (cierre EEUU), L-V, en `update_all.yml`. La comprobación de alertas de watchlist va 30 min después (17:30 / 22:30).
- **Frescura de precios** (`lib/market-days.js`, testeado): `expectedCloseDate()` = último cierre que YA debería existir (día hábil anterior hasta las 22:00 UTC). `/api/precios` refresca contra ese criterio (antes toleraba 2 días → un precio del lunes valía el miércoles) y devuelve `fresh` por ticker; `shouldReplacePrice` impide que un cierre viejo de daily_prices pise el `current_price` más reciente del scrape semanal en cartera/home.
...
- `projectIncome` (lib/portfolio-calc.js) — proyección a 10 años; **el año 1 == `calcFiscal`** (impuesto por país, sin crecer todavía; el CAGR se aplica desde el año 2). Acepta `whtOverrides`. Alimenta SOLO la página `/cartera/proyeccion` (escenarios).
- **Gráfico "Renta neta por dividendos, € (real por año)"** de `/cartera` (`IncomeProjectionCard`): muestra la renta REAL por año natural desde `dividends_received` (cobrado sólido + estimado del año en curso translúcido, apilados). Llama al prefill al cargar. NO es proyección de crecimiento: cada año refleja lo realmente cobrado; el año en curso = cobrado + lo que resta con las acciones ACTUALES (= "Total año natural" de la pestaña Dividendos). Cuadra con esa pestaña, no con el ritmo anualizado.
...
- `computeAutoDividends` (lib/dividends.js) — **año NATURAL** (cobrado + estimado, *con gating por fecha de compra*: no incluye pagos anteriores a la compra). Por eso para compras a mitad de año es legítimamente MENOR que el ritmo anual. Etiqueta de la pestaña Dividendos: "Total año natural {year}". **`period` = mes de PAGO** (no el ex), igual que `payment_date_estimated`; debe coincidir con cómo el prefill calcula `taken`/exclusiones — si no, los dividendos cuyo ex y pago caen en meses distintos (Inditex, Munich Re) reaparecían tras borrarlos. Al borrar un auto, la casilla "No volver a generar" viene marcada por defecto.
...
- **PortfolioHome** (client, carga la cartera como PortfolioPage: posiciones+fundamentales+precios+ajustes+liquidez+notificaciones+FX). 5 bloques centrados en interés compuesto y progreso:
  ...
  - **Badges de hitos** (`MilestoneBadges` + `lib/milestones.js`, testeado): logros que motivan — Patrimonio (1k…1M), Renta anual neta (100…25k/año), Libertad financiera (10…100%), Diversificación (3…50 empresas). Muestra el nivel actual + barra al siguiente. **Celebra** con banner "🎉 ¡Enhorabuena!" el hito nuevo MÁS relevante (`newlyReached`), comparando con lo ya visto en `localStorage` (`everdiv:milestones:v1`); la 1ª carga registra lo histórico sin celebrarlo.
...
- **Estimaciones de analistas (FMP)** (`components/empresa/AnalystEstimates.js` + `lib/analyst-estimates.js` + `app/api/empresa/[ticker]/estimates`): tabla + gráfico combinado (barras de ingresos + línea de BPA, real sólido / estimado discontinuo) con un continuo años reales (de `income_statement_annual`) → estimados (consenso FMP). Ingresos en millones (M), como el resto de la app. La **fuente (FMP) se oculta** a propósito en la UI.
  ...
  - **Fallback no-US con Yahoo** (`lib/yahoo-estimates.js`): si FMP no da estimaciones, la ruta pide el módulo `earningsTrend` de Yahoo (`fetchYahooEstimates`), que SÍ cubre muchas empresas fuera de EE.UU. (Inditex, BMW, Airbus…). Devuelve la misma forma `[{year,revenue,eps,analysts}]` (años `0y`+`+1y`, año desde `endDate`) → `buildEstimateSeries` las combina igual. En vivo (no precargado), con la caché de 24h de la ruta. Comprobado: Finnhub free NO sirve (financieros solo US, estimaciones de pago). Fuente oculta en la UI.
...
Sección de guías educativas para tráfico orgánico: `/guias` (índice) + `/guias/[slug]` (artículo). Contenido en `data/guias.js` (bloques h2/p/ul/callout/cta con markdown inline mínimo `[texto](/url)` y `**negrita**`), renderizado por `components/GuiaContent.js`. Cada artículo lleva `generateMetadata` (title/description/canonical/openGraph) + JSON-LD `Article` + relacionadas + CTA de registro, y enlaza internamente a screener/aristocratas/cartera. En el `sitemap.js` y en el footer de la landing. 4 guías iniciales (DGI, fiscalidad ES, Reyes/Aristócratas, métricas). **Editor en el panel admin** (`/dashboard/guias`, `GuiasEditorClient`): las guías viven en la tabla `guias` (SQL `guias.sql`, **pendiente**; contenido en Markdown subset — `##`/`-`/`>`/`!cta` + inline `[t](/u)`/`**b**`, parseado por `lib/markdown-blocks.js`). Crear/editar/publicar con vista previa en vivo (reusa `GuiaContent`), guardar borrador o publicar. API `/api/admin/guias` (GET/POST/DELETE, admin). Las páginas públicas leen de BD (`lib/guias-db.js`) y **caen a las estáticas de `data/guias.js` si la tabla está vacía**. Seed inicial: `node scripts/seed_guias.mjs --write` (convierte las estáticas a Markdown → tabla). El sitemap lee los slugs publicados de BD.
...
`roic_display.sql`, `funds_returns.sql` (incl. benchmark_name), `cancellations.sql`, `onboarding.sql`, `watchlist.sql` (tablas watchlist + notifications), `yield_avg.sql` (columnas yield_avg + yield_avg_years), `ma200.sql` (columna MM200), `score_history.sql` (tabla de histórico del Score DGI), `canibales.sql` (columnas shares_reduced_pct + shares_base_year), `compounders.sql` (columna capex_cfo_pct), `bank_metrics.sql` (tabla bank_metrics_manual — NPL/overrides bancarios por trimestre), `analyst_estimates.sql` (columnas analyst_estimates + _status + _at — necesario antes de correr fetch_analyst_estimates.mjs/workflow), `positions_commission.sql` (columna positions.commission — override de comisión por posición desde el editor de la cartera), `cash_fund.sql` (tabla cash_movements + columnas user_settings.cash_interest_rate/dividends_to_cash — Fondo de oportunidad), `monthly_expenses.sql` (columna user_settings.monthly_expenses — contador de libertad de la home), `guias.sql` (tabla guias — editor de artículos del blog), `paddle.sql` (columnas paddle_subscription_id/paddle_customer_id en user_settings — pago Paddle), y el ALTER de `premium_until` en user_settings. NOTA: el bucket de Storage `company-logos` ya está creado por `fetch_logos.mjs` (no es SQL).
Nuevos de esta sesión: `earnings_dates.sql` (columna next_earnings_date — CONFIRMADO ejecutado + backfill corrido), `purchase_plan.sql` (tabla del Calendario de compras — CONFIRMADO ejecutado), `financial_history.sql` (tabla del backfill de histórico financiero — CONFIRMADO ejecutado), `user_events.sql` (tabla de tracking de actividad — PENDIENTE de confirmar; el proxy y el dashboard son tolerantes si no existe), `financial_history_quarterly.sql` (histórico trimestral permanente acumulativo — PENDIENTE de ejecutar; tras ejecutarlo correr `python scripts/backfill_quarterly.py --write`).
Si se monta un entorno nuevo, ejecutar en orden todos los ficheros de webapp/sql/.

## Backfill puntual de histórico financiero (financial_history) — NO es el cron
- Objetivo: máximo histórico posible de estados financieros completos + dividendo/acción + acciones, para las ~2000 empresas, de una sola ejecución MANUAL (no toca `update_fundamentals.py` ni el cron). Los US ya quedan con ~20 años; el problema es la profundidad no-US.
- Tabla **`financial_history`** (`webapp/sql/financial_history.sql`, ejecutada): estados completos + `raw_concepts` jsonb, **`unique(ticker, fiscal_year, source)`** → cada fuente guarda sus años como filas independientes (modelo **ACUMULATIVO**, no "primer éxito": un ticker no-US pasa por todas las fuentes y se suman los años). RLS solo service_role (scripts escriben, dashboard lee).
- Fuentes (cascada acumulativa): **SEC EDGAR** (`scripts/backfill_sec_edgar.py`, oficial, us-gaap/ifrs-full 10-K/20-F, ~615 empresas US+ADR a ~13-20a — la principal) · **stockanalysis** (`scripts/backfill_stockanalysis.py`, scraping, ~5a no-US; `--file lista.json` procesa una lista de tickers concreta, p.ej. los falsos matches borrados de SEC) · **Macrotrends** (`scripts/backfill_macrotrends.py`, curl_cffi — **BLOQUEADO por Cloudflare desde datacenter Y desde IP residencial**; requiere navegador headless, valor incremental bajo → descartado de momento) · **ESEF (UE)** y **EDINET (Japón)** oficiales — pendientes de construir (EDINET necesita API key gratuita del usuario).
- **Estado actual**: ~15.400 filas / **~1.575 empresas** distintas (SEC ~10.600, stockanalysis ~4.800). Los 252 falsos matches borrados de SEC se reprocesaron con stockanalysis por mercado (159 recuperados con datos correctos).
- **BUGS del backfill SEC ya corregidos** (destapados por el validador): (1) indexar por año de FIN de periodo (`end`), NO por `fy` (que es el del informe → un 10-K trae comparativas con el mismo fy); (2) falsos matches de CIK en no-US (quitar el sufijo casa `ADM.L` con ADM US) → se limpian con el validador.
- **Validación cruzada** `scripts/validate_financial_history.py`: contrasta cada empresa contra yfinance (`income_statement_annual`) en los años solapados. **yfinance MANDA SIEMPRE** (prioridad yfinance > sec_edgar > stockanalysis > macrotrends, ver `SOURCE_PRIORITY`). Clasifica OK/WARN/FAIL/SIN_REF. Solo **reporta** (`scripts/output/validation_report.json`); el borrado es manual. **Criterio de limpieza de stockanalysis**: los FAIL en **financieras (banca/seguros)** se MANTIENEN (el "revenue" difiere por definición —primas netas/brutas, interés neto/bruto—, la empresa es correcta y el BPA/balance es válido); los FAIL en **no financieras** (>25% de diferencia de ingresos) se BORRAN (revenue debería casar si es la misma empresa/base → evita barras de ingresos antiguas inconsistentes con yfinance).
- **Consumo en la ficha**: `lib/financial-history-merge.js` (`extendStatements`) extiende `income/cashflow/balance_annual` de yfinance con los años ANTIGUOS del backfill (yfinance intacto en solapados) y la ficha pasa los estados EXTENDIDOS a `FinanzasDeepDive` + `buildPeHistory` + `buildEvEbitdaHistory` — SOLO a visualización, NO al `detail` de scoring/valoración. Se lee con service_role, solo premium. (AAPL: 4 → 20 años.)
- **Dashboard**: pestaña **Datos → "Cobertura histórica"** (`components/dashboard/CoberturaHistoricaClient.js` + `/api/admin/coverage` + `lib/financial-history-stats.js` `getCoverageByCompany`/`getUncoveredCompanies`): años cubiertos, fuente y completitud por bloque (IS/BS/CF/Acc) por empresa + filtro de sin-datos.

## Ampliaciones de la ficha de empresa (6 features, todas premium)
Añadidas a las pestañas existentes (Resumen/Dividendo/Valoración), calculadas en `page.js` (server) y pasadas como props a `CompanyDetailPage`. Cada lib es pura + testeada (`test/ficha-extras.test.mjs`, `test/backtest.test.mjs`).
- **Posición dentro del sector** (`lib/sector-percentiles.js` + `components/empresa/SectorPositioning.js`, Resumen): percentil de ROIC/rentabilidad/crecimiento div/crecimiento ingresos/margen operativo/payout/deuda/racha frente a los **peers del MISMO sector** (`page.js` hace un fetch de `company_fundamentals` filtrado por `sector`). `percentileRank(value, arr, higherBetter)` (menor es mejor para payout/deuda). Payout comparable = `payout_eps` preferido (menos artefactos que FCF). "top X%" si top≤25, si no "Pxx".
- **Precio de compra DGI** (`lib/buy-price.js` + `BuyPriceCard.js`, Valoración): precio de entrada objetivo = valor intrínseco × (1−`MOS_TARGET` 20%), gated por Score DGI ≥ `MIN_QUALITY` 6.5. Muestra distancia al precio actual, si ya está en zona, MoS actual y precio de referencia por yield medio histórico. Botón directo a la alerta de watchlist (`PriceAlertButton`). Si calidad baja o sin valoración fiable → lo explica.
- **Resiliencia del dividendo en crisis** (`lib/dividend-resilience.js` + `DividendResilience.js`, Dividendo): usa `div_history` (llega décadas atrás) para ver si mantuvo/subió/recortó el dividendo en 2008-09, 2020 y 2000-02. Solo crisis donde ya repartía. "Superó N de M sin recortarlo".
- **¿Está mejorando o empeorando?** (`lib/quality-trend.js` + `QualityTrend.js`, Resumen): flechas ↑/→ por dimensión desde las bonificaciones por tendencia (`bonus_*`, valor>0 = mejorando) + delta del Score DGI desde `score_history`.
- **Backtest "¿y si hubieras invertido?"** (`lib/backtest.js` + `BacktestCard.js`, Resumen): 1.000 € hace 3/5 años → valor hoy con dividendos reinvertidos, CAGR total vs solo precio, dividendos cobrados, aporte del dividendo. Cliente: reutiliza el endpoint `/api/empresa/[ticker]/chart` + la MISMA reconstrucción de total return del `PriceChart`.
- **Sensibilidad de la valoración** (`ValuationSensitivity.js`, Valoración, solo motor `dcf`): heatmap 5×5 del margen de seguridad variando crecimiento (`g1`) × tasa de descuento (`r`) vía `recomputeValuation`. Verde infravalorada / rojo sobrevalorada; centro = escenario base.

## Histórico trimestral permanente + TTM/YoY (capa adicional)
- **Problema resuelto**: `company_fundamentals.*_quarterly` es una **ventana viva** que se SOBRESCRIBE cada semana (antes 4 trimestres → **ampliada a 8**), así que los trimestres antiguos se perdían. Ahora se conservan **para siempre** en una tabla acumulativa.
- **Tabla `financial_history_quarterly`** (`webapp/sql/financial_history_quarterly.sql`, **PENDIENTE de ejecutar**): `unique(ticker, period, source)`, `period` = fecha de cierre del trimestre `YYYY-MM-DD`. `update_fundamentals.py` (`upsert_quarterly_history` + `build_quarterly_rows`) hace **UPSERT acumulativo** tras el upsert principal — NUNCA borra; los trimestres fuera de la ventana viva permanecen. RLS service_role only. Backfill inmediato: `scripts/backfill_quarterly.py --write` (reutiliza los helpers, yfinance da ~5-6 trimestres ahora → habilita YoY del/los trimestre(s) reciente(s); la acumulación semanal llega a 8+). Futuro: backfill profundo desde SEC 10-Q.
- **Decisión de alcance**: es una **CAPA ADICIONAL** — NO toca valoración/ROIC/Score canónicos (siguen sobre datos anuales). Solo añade TTM y gráficos.
- **Lógica pura** `lib/quarterly.js` (testeada, `test/quarterly.test.mjs`): `fromHistoryRows`/`fromLiveStatements` → serie ascendente; `buildQuarterlyModel` → por trimestre (márgenes + YoY emparejando el mismo trimestre del año anterior por fecha −1a ±50d, robusto a cadencias irregulares), **TTM rodante** (suma móvil de 4T), **TTM actual vs anterior** (YoY del TTM) y resumen del último trimestre.
- **UI** `components/empresa/QuarterlyTrends.js` (ficha → pestaña Finanzas, tras StatementCharts, **premium**): 4 vistas — (1) resumen YoY del último trimestre + línea TTM, (2) ingresos/beneficio por trimestre con línea de % YoY, (3) tendencia TTM (ingresos/FCF), (4) márgenes por trimestre. `page.js` lee `financial_history_quarterly` (service_role, premium) → prop `quarterlyRows`; el componente cae al JSON vivo (`income/cashflow_quarterly`) si la tabla aún está vacía. Estado "acumulando histórico" si <2 trimestres.

## Tracking de actividad de usuarios (dashboard → Actividad)
- Tabla **`user_events`** (`webapp/sql/user_events.sql`, PENDIENTE de confirmar): RLS solo INSERT propio (`auth.uid()=user_id`), sin SELECT; lectura/limpieza por service_role. Índices `(user_id,section)` y `created_at`.
- Captura en **`proxy.js`** (server-side, sin JS cliente): `event.waitUntil()` fire-and-forget + try/catch silencioso, reutiliza el `supabase`/`user` ya existentes. `lib/event-tracking.js` mapea ruta→sección por **lista blanca** (páginas GET + acciones clave POST: `comparacion_hecha`, `watchlist_accion`, `aportacion_procesada`; ticker en metadata para la ficha).
- Agregación `lib/activity-stats.js` (`getSectionUsage`/`getDailyActiveUsers`/`getActiveAccounts`, paginado, email vía `auth.admin.listUsers`). UI `components/dashboard/ActividadClient.js` (uso por sección + DAU recharts + cuentas activas). Retención 90d añadida a `/api/admin/clean-logs` (junto a admin_logs).

## Calendario de compras (`/cartera/calendario-compras`)
- Planificar aportaciones futuras y proyectar la cartera. Tabla **`purchase_plan`** (`webapp/sql/purchase_plan.sql`, ejecutada, RLS por user_id, CRUD directo cliente como positions). Cola CRUD (ticker/empresa aunque no esté en cartera, fecha objetivo, importe o acciones, rango de precio en divisa de la acción, notas, estado pendiente/ejecutada/descartada; chip "en watchlist").
- Proyección "si ejecuto el plan" (6/12/24m): reutiliza `enrichPositions`+`calcConcentration` sobre posiciones HIPOTÉTICAS (`lib/purchase-plan.js` `buildHypotheticalPositions`); peso por posición con banda equiponderación 4-6%, por sector/país/divisa. Avisos de secuencia: X meses sin abrir posición nueva (desde transactions) + entrada que supera umbral de concentración (config en localStorage). `components/cartera/CalendarioComprasClient.js` + ítem en `CarteraNav`.

## Fechas de resultados (next_earnings_date)
- Columna `next_earnings_date` (`webapp/sql/earnings_dates.sql`, ejecutada + backfill `scripts/backfill_earnings_dates.mjs`): capturada en `update_fundamentals.py` (info.earningsTimestampStart) y del calendario Yahoo (`fetchYahooEarningsDate` en `lib/yahoo-estimates.js`). Se muestra en la cabecera de la ficha ("Próximos resultados"), en Novedades ("Presentan resultados esta semana") y en el resumen mensual por email.

## Sesión reciente — cambios de precisión / correcciones
- **Yield/CAGR con el año EN CURSO**: `dps` y `div_cagr5` usan el reparto del año en curso (entrada isPartial de div_history) si ya SUPERA al último año completo (subida materializada) — en `update_fundamentals.py` (dps + compute_div_cagr5) y backfill `fix_dps_from_history.mjs`. Guard >40% intacto. Ver memoria `yield-dps-from-history`.
- **Salud financiera**: (1) margen operativo del semáforo/tarjeta por INDUSTRIA vía `omBreaks(detectIndustryType)` (exportados de dgi-score) — salud y score coherentes, salvo REIT/utilities/banca. (2) Payout OCF (REIT) / CFO (utilities) = 100/(CFO/dividendos) vía `exCfoDivCoverage` (no `payout_fcf`). En `lib/health.js`.
- **Valoración REIT**: el cross-check relativo usa **P/AFFO mediana del sector** (no PER, inválido en REITs) — `sectorMedianPAffo` en la ficha + `pAffoRelativeValue` en `lib/valuation-methods.js`. Derivado de `payout_affo`+`dps`+`current_price`.
- **Valoración — divergencia FCF↔ingresos** (`businessGrowth` en `lib/valuation.js` + espejo Python): si el FCF se desploma (`rev>0 && rev-fcf>=15 && fcf<-5`) mientras los ingresos crecen, usa ingresos como proxy (no el promedio que colapsaba el valor). Arregla KO (−86%→−65%). Ver memoria `valuation-fcf-divergence`.
- **Valoración — momentum reciente (trimestres) + suelo anti-decrecimiento** (`businessGrowth` JS + `_business_growth` Python, espejo): el DCF ya no depende SOLO del CAGR anual de 5 años (que puede estar estancado mientras el negocio crece de nuevo). (1) Se **mezcla al 50%** el crecimiento interanual RECIENTE (`_recentRevYoY`: TTM vs TTM anterior si hay ≥8 trimestres, si no último trimestre vs el mismo del año anterior con ≥5; recortado a ±15%). (2) **Suelo**: un negocio con ingresos creciendo (a 5a o reciente) no se valora con decrecimiento perpetuo (`gPct = max(gPct, 0)`). Caso Apple: growth −1% → +7%, valor intrínseco 72→~132. El crecimiento reciente lo inyecta la ficha (`page.js` desde `financial_history_quarterly` antes de `computeValuation`) y el script (`update_fundamentals.py` desde `q_income`, `_recent_rev_yoy`). `scripts/recalc_valuation.py` lee el YoY reciente de `financial_history_quarterly` (`load_fhq_yoy`) → ya reejecutado con `--write` (2496 filas). Test `valuation-growth.test.mjs` (`businessGrowth` exportada).
- **Comparador**: MoS/valor intrínseco no fiables (mosUnreliable / |mos|>85) se ocultan (KO ya no muestra −86%). Tooltip en "Regla 10/10". Nota en "Renta acumulada 10 años" (solo dividendos).
- **Import de dividendos (ING)**: la retención en ORIGEN = tasa estatutaria del país (`getWHT(country, wht_overrides)`), destino = resto; ES → origen 0. Antes fijaba destino=19% y derivaba origen (descuadraba: Sanofi 20,22% en vez de 25%). En `lib/broker-import.js` (`computeDividend`). Test `broker-import`.
- **Fiscalidad**: casilla 0031 = componente español (destino) de todos los dividendos (antes dejaba fuera la retención de las españolas, que vive en withholding_dest). Retención española **recuperable** para exentos (`isExemptUser`) se muestra en Dividendos (suma a la rentabilidad real). Añadir dividendos manuales de empresas NO en cartera (buscador abierto a todo el DICT).
- **Gráfico evolución del patrimonio** (`/api/cartera/evolucion`): `normalizeGbp` al precio de mercado de las `.L` (peniques→libras) — antes ×100 rompía junio.
- **Resumen mensual por email** (`/api/resumen-mensual`): renta = "Total año natural" (suma neta de dividends_received del año, cuadra con /cartera), dividendos COBRADOS del mes, sección "Dividendo en peligro" (recortes + `calcDividendRisks`), "Presentan resultados en \<mes\>" (fecha exacta en vivo). Modo prueba `?email=` (admin, CRON_SECRET) + `&debug=1`. Charset utf-8 + dominio www.everdiv.com en los emails.
- **Lanzamiento**: páginas legales `/privacidad` (identidad: Manuel Herrero, 70889796R), `/terminos` (descargo financiero), `/cookies` + aviso `CookieNotice` (`lib/legal` no; `components/LegalDoc.js`). `CRON_SECRET`, `RESEND_API_KEY` (dominio everdiv.com verificado) y Paddle configurados en Vercel prod. Dominio **www.everdiv.com**.
- **Dashboard rediseñado**: layout con `var(--bg)` (antes #080b14 hardcodeado, rompía tema claro), kit UI (`components/dashboard/ui.js`) pulido (Card radio 14+sombra, MetricCard con barra de acento, PageTitle con divisor), sidebar con pastilla de activo + logo.

## Infografía en PDF (comparador + ficha) — Premium
- Exporta una "hoja de comparación financiera" en PDF, estilo referencia (`5994711908205924038.jpg`, en la raíz). **Comparador = A4 horizontal a 3 columnas**: empresa A (izq., color de marca) + tabla financiera central + empresa B (dcha.); cada lateral con **Negocio / Fortalezas / Riesgos / Resultados**; central con tabla financiera (iconos), **Calidad del negocio** (nota /10 + estrellas) y **Resumen rápido** (checkmarks); franja inferior con **Puntuación final /10** (2 columnas, resalta la ganadora), **¿Cuál compraría hoy?** (trofeo + veredicto) y **Dividendos** (estrellas comparadas). **Ficha = A4 vertical** de una columna (misma estética). Estrellas de dividendo: rentabilidad/seguridad/crecimiento histórico/prob. de seguir subiendo. Veredicto **derivado solo de datos**. Disclaimer legal al pie. **Sin logos** (marcas registradas): solo ticker, nombre y color.
- **Generación**: HTML/CSS (string) → **Puppeteer + `@sparticuz/chromium`** en el runtime Node serverless de Vercel. `lib/pdf.js` (`htmlToPdf`) reutiliza una única instancia de navegador; `preferCSSPageSize:true` → la orientación la fija el `@page` de cada plantilla (landscape comparador / portrait ficha). En local usa el Chrome del sistema. `next.config.mjs`: `serverExternalPackages` + `outputFileTracingIncludes` del `bin/` de chromium. **Fix Vercel**: `@sparticuz/chromium` solo extrae sus librerías (`libnss3`) y fija `LD_LIBRARY_PATH` si detecta AWS Lambda por `AWS_EXECUTION_ENV`/`AWS_LAMBDA_JS_RUNTIME`; en Vercel no coinciden → se fuerza `AWS_LAMBDA_JS_RUNTIME='nodejs20.x'` (rama AL2023) **antes** de importar el módulo (import dinámico en `launchServerless`).
- **Datos (fuente única)**: `lib/infografia-data.js` `buildInfographicModels(tickers)` reutiliza `buildComparadorCompanies` (mismos scores/sub-scores/yield) y lo enriquece con `dividendSafety`, `yield_avg` (vs media histórica), `shares_reduced_pct` (recompra), **`data/profiles.js`** (descripción "Negocio"), `buildStrengths`/`buildRisks` (Fortalezas/Riesgos derivados de datos), `market_cap_m`, `next_earnings_date`/`last_report_period` (Resultados). **Fiscalidad ES fija** (`ES_DEST_WHT=19`): yield **bruto y neto** (neto vía `netYield` con tope doble imposición 15%). Colores de columna por posición (`COLUMN_COLORS`, A azul / B rojo, estilo referencia). Veredicto: `buildVerdict` / `buildSingleVerdict` + `buildCommonNotes` (resumen rápido) — solo datos. NOTA: la puntuación del PDF usa el motor del screener (`computeScore`), coherente con el comparador; puede diferir del `computeDGIScore` de la ficha.
- **Plantilla** en `lib/infografia-template.js` (separada, reutilizable): `renderComparadorHtml(models)` y `renderEmpresaHtml(model)`.
- **Endpoints** `GET /api/infografia/comparador?tickers=A,B` y `GET /api/infografia/empresa?ticker=A` (`runtime='nodejs'`, `maxDuration=60`). **Premium-gated** (403 si no; free ve CTA). **Caché en memoria** del Buffer PDF por clave (tickers) TTL 12h + `Cache-Control: private, max-age=43200`.
- **Botón** `components/InfografiaButton.js` (`kind="comparador"|"empresa"`): fetch → blob → descarga; si 403, muestra aviso + enlace a `/pricing`. Montado en `ComparadorClient` (junto a CSV/PNG, solo con ≥2 empresas) y en la cabecera de la ficha (`CompanyDetailPage`, junto a "Comparar →"). Tests: `test/infografia.test.mjs` (veredictos, solo datos).

## Oferta beta fundadores (Paddle) + Reyes gratis
- **Oferta**: los 100 primeros suscriptores → **20 €/año bloqueado de por vida**. Anunciada en la landing (`BetaBanner` + sección `BetaOffer` + CTA → `/fundador`).
- **Pago por Paddle Billing** (merchant of record — gestiona el IVA de la UE). `/fundador` (`FundadorClient`, requiere sesión) carga Paddle.js del CDN (sin dep npm) y abre el checkout overlay con el price de fundador, pasando `customData:{user_id}`. Env: `NEXT_PUBLIC_PADDLE_CLIENT_TOKEN`, `NEXT_PUBLIC_PADDLE_ENV` (sandbox|production), `NEXT_PUBLIC_PADDLE_PRICE_FOUNDER`, `PADDLE_WEBHOOK_SECRET`.
- **Webhook** `app/api/paddle/webhook/route.js`: verifica la firma HMAC (`Paddle-Signature`), y por `subscription.*`/`transaction.completed` **concede/renueva/revoca Premium** en `user_settings` (plan/premium_until + `paddle_subscription_id`/`paddle_customer_id`, SQL `paddle.sql`). Usuario por `custom_data.user_id` (o `paddle_customer_id` en renovaciones). Activo → premium_until = fin de periodo + 3d de gracia; cancelado → hasta fin de periodo. Es recurrente (renueva solo), a diferencia del Bizum manual anterior (eliminado: `/api/beta/*`, `beta_requests` admin, `NEXT_PUBLIC_BIZUM_PHONE`).
- **PENDIENTE (setup Paddle)**: crear cuenta Paddle + producto/price 20 €/año, poner las 4 env vars en Vercel, configurar el webhook a `/api/paddle/webhook`, y ejecutar `paddle.sql`. El límite de 100 plazas se controla despublicando el price en Paddle cuando se alcance.
- **Reyes del dividendo GRATIS** (gancho): en `app/empresa/[ticker]/page.js`, si `div_streak ≥ 50` y el usuario es free → `isKingFree=true` → `isPremium=true` (ficha COMPLETA gratis, con banner 👑). Escaparate del análisis Premium antes de suscribirse. Es dinámico (cualquier empresa que alcance 50 de racha entra).
